...
**IMPORTANT: Before connecting to Lightpanda, ALWAYS ensure it is running.** Run this check before any CDP connection:

```bash
# Check if Lightpanda is already running on port 9222, start if not
if ! curl -s http://127.0.0.1:9222/json/version > /dev/null 2>&1; then
  nohup $HOME/.local/bin/lightpanda serve --host 127.0.0.1 --port 9222 > /tmp/lightpanda.log 2>&1 &
  sleep 1
  # Verify it started
  if curl -s http://127.0.0.1:9222/json/version > /dev/null 2>&1; then
    echo "Lightpanda started (PID $!)"
  else
    echo "ERROR: Lightpanda failed to start. Check /tmp/lightpanda.log"
  fi
else
...
This ensures Lightpanda starts automatically on demand without manual intervention. The process runs in the background and persists for the duration of the session.
...
## Start the Browser Server (Manual)
```bash
